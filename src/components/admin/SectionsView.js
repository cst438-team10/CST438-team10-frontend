...
function SectionsView(props) {
    ...
    const fetchSections = async () => {
        if (search.courseId==='' || search.year==='' || search.semester==='' ) {
          ...
          try {
            const response = await fetch(`${SERVER_URL_REGISTRAR}/courses/${search.courseId}/sections?year=${search.year}&semester=${search.semester}`);
            if (response.ok) {
    ...
    const deleteSection = async (secNo) => {
      try {
        const response = await fetch (`${SERVER_URL_REGISTRAR}/sections/${secNo}`, 
        {
          method: 'DELETE',
          headers: {
